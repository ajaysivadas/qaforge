...
function banner() {
  ...
  log(
    `${c.cyan}${c.bold}  ║           QA PILOT v${VERSION}            ║${c.reset}`
  );
  log(
...
function installKnowledge(targetBase, label) {
  ...
  const knowledgeDest = path.join(targetBase, "qa-pilot-knowledge");
...
function scanAndGenerateContext() {
  ...
  context.push(`# QA Pilot - Project Context`);
  context.push(`# Auto-generated by qa-pilot scan`);
...
function main() {
  ...
  if (isHelp) {
    ...
    log(`    npx qa-pilot              Interactive install`);
    log(`    npx qa-pilot --global     Install to ~/.claude/ (all projects)`);
    log(`    npx qa-pilot --local      Install to ./.claude/ (this project)`);
    log(
      `    npx qa-pilot --scan       Scan project and generate context file`
    );
    log(`    npx qa-pilot --help       Show this help`);
    log(`    npx qa-pilot --version    Show version`);
    log("");
  ...
  if (isScan) {
    ...
    const outPath = path.join(process.cwd(), ".claude", "qa-pilot-context.md");
    ensureDir(path.dirname(outPath));
    fs.writeFileSync(outPath, context);
    log(
      `${c.green}  [OK]${c.reset} Project context written to ${c.bold}.claude/qa-pilot-context.md${c.reset}`
    );
    log(
      `${c.dim}  This file helps QA Pilot commands understand your project.${c.reset}`
    );
    log("");
    process.exit(0);
  }
  ...
  if (isGlobal || isAuto) {
    ...
    installKnowledge(CLAUDE_GLOBAL, "~/.claude/qa-pilot-knowledge/");
    installed = true;
  }
  ...
  if (isLocal) {
    ...
    installKnowledge(CLAUDE_LOCAL, ".claude/qa-pilot-knowledge/");

    // Also scan the project
    log(`\n${c.cyan}  Scanning project for context...${c.reset}`);
    const context = scanAndGenerateContext();
    const outPath = path.join(CLAUDE_LOCAL, "qa-pilot-context.md");
    fs.writeFileSync(outPath, context);
    log(
      `${c.green}  [OK]${c.reset} Project context written to ${c.bold}.claude/qa-pilot-context.md${c.reset}`
    );
  ...
  if (!isGlobal && !isLocal && !isAuto) {
    ...
    fs.writeFileSync(outPath, context);
    log(
      `${c.green}  [OK]${c.reset} Project context saved to .claude/qa-pilot-context.md`
    );
    installed = true;
  }
  ...
  if (installed) {
    ...
    log("");
    log(
      `${c.dim}  Tip: Run 'npx qa-pilot --scan' inside any project to generate context.${c.reset}`
    );
    log("");
  }
}
